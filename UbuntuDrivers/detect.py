...
def check_nvidia_module_status():
    ...
    try:
        ...
        result['next_boot_kernel'] = os.uname().release
            ...
            # Check if next boot kernel is different from current
        current_kernel = os.uname().release
        if result['next_boot_kernel'] and result['next_boot_kernel'] != current_kernel:
            result['needs_reboot'] = True
            
            # Check if nvidia module exists for next boot kernel
            if result['next_boot_kernel']:
                # Check multiple possible paths for NVIDIA modules
                possible_paths = [
                    # Standard kernel module path
                    f'/lib/modules/{result["next_boot_kernel"]}/kernel/drivers/video/nvidia.ko',
                    # Compressed standard module
                    f'/lib/modules/{result["next_boot_kernel"]}/kernel/drivers/video/nvidia.ko.gz',
                    # DKMS module path (common for NVIDIA drivers)
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/dkms/nvidia.ko',
                    # Compressed DKMS module
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/dkms/nvidia.ko.gz',
                    # Zstandard compressed DKMS module (like your system)
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/dkms/nvidia.ko.zst',
                    # Alternative DKMS paths
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/nvidia.ko',
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/nvidia.ko.gz',
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/nvidia.ko.zst',
                    # Additional DKMS paths used in some distributions
                    f'/lib/modules/{result["next_boot_kernel"]}/extra/nvidia.ko',
                    f'/lib/modules/{result["next_boot_kernel"]}/extra/nvidia.ko.gz',
                    f'/lib/modules/{result["next_boot_kernel"]}/extra/nvidia.ko.zst',
                    # Check for any nvidia.ko* file in the kernel modules directories
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/dkms/',
                    f'/lib/modules/{result["next_boot_kernel"]}/updates/',
                    f'/lib/modules/{result["next_boot_kernel"]}/extra/'
                ]
                ...
                for path in possible_paths:
                    if os.path.exists(path):
                        if path.endswith('/'):  # Directory path
                            # Check if directory contains nvidia.ko* files
                            try:
                                for file in os.listdir(path):
                                    if file.startswith('nvidia.ko'):
                                        result['next_boot_module_path'] = os.path.join(path, file)
                                        module_found = True
                                        break
                                if module_found:
                                    break
                            ...
                            result['next_boot_module_path'] = path
                            module_found = True
                            break
                
                if not module_found:
                    result['module_missing'] = True
                    # Add debugging information about what was checked
                    result['debug_info'] = {
                        'checked_paths': possible_paths,
                        'next_boot_kernel': result['next_boot_kernel']
                    }
